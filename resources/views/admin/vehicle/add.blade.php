		...
		<div class="row">
			<div class="col-md-12">
				<div class="box box-info">
					...
					{!! Form::hidden('user_country_code', $driver->country_code, ['id' => 'user_country_code']) !!}
					{!! Form::hidden('user_gender', $driver->gender, ['id' => 'user_gender']) !!}
					{!! Form::hidden('driver_name', $driver->first_name.' '.$driver->last_name, ['id' => 'first_name']) !!}
					@if (LOGIN_USER_TYPE!='company')
						{!! Form::hidden('company_name', $driver->company_id, ['id' => 'company_name']) !!}
					@else
						<span ng-init='company_name="{{Auth::guard("company")->user()->id}}";vehcileIdList=[];get_driver()'></span>
					@endif


					{!! Form::hidden('driver_id', $driver->id, ['id' => 'driver_id']) !!}
					<div class="box-body">
...
{{--						</div>--}}
						...
						<div class="form-group">
			              ...
			              <div class="col-md-7 col-sm-offset-1">
			                {!! Form::select('vehicle_make_id',$make, '', ['class' => 'form-control',"onchange"=>"addOtherMake(this)", 'id' => 'vehicle_make', 'placeholder' => 'Select']) !!}
			                <span class="text-danger">{{ $errors->first('vehicle_make_id') }}</span>
			              </div>
			            </div>
						<div class="form-group" id="other_make" style="{{(empty($errors->first('other_make'))) ? 'display: none' : ''}}">
							<label for="vehicle_number" class="col-sm-3 control-label"><em class="text-danger">*</em></label>
							<div class="col-md-7 col-sm-offset-1">
								{!! Form::text("other_make",'', ['class' => 'form-control', 'id' => 'other_vehicle_make', 'placeholder' => 'Add vehicle make']) !!}
								<span class="text-danger">{{ $errors->first('other_make') }}</span>
							</div>
						</div>
			            <div class="form-group">
			              ...
			              <div class="col-md-7 col-sm-offset-1">
			                ...
			                {!! Form::select('vehicle_model_id',array(), '', ['class' => 'form-control',"onchange"=>"addOtherModel(this)", 'id' => 'vehicle_model', 'placeholder' => 'Select']) !!}
			                <span class="text-danger">{{ $errors->first('vehicle_make_id') }}</span>
			              </div>
			            </div>
						<div class="form-group" id="other_model" style="{{(empty($errors->first('other_model'))) ? 'display: none' : ''}}">
							<label for="vehicle_number" class="col-sm-3 control-label"><em class="text-danger">*</em></label>
							<div class="col-md-7 col-sm-offset-1">
								{!! Form::text("other_model",'', ['class' => 'form-control', 'id' => 'other_vehicle_model', 'placeholder' => 'Add vehicle model']) !!}
								<span class="text-danger">{{ $errors->first('other_model') }}</span>
							</div>
						</div>
		...
		$(document).ready(function() {
			...
			$('#vehicle_make').on('change', function(e){
				...
				$.ajax({
					...
					data: {make_id:make_id,"_token": "{{ csrf_token() }}"},
