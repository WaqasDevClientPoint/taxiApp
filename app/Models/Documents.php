  ...
  */
  ...
  public function scopeDocumentCheck($query,$document_for,$country_code){
    if($country_code !='all'){
      ...
      return $query->where('type',$document_for)->whereIn('country_code',[$country_code,'all'])->select('id','document_name','expire_on_date','country_code','required');
    }else{
      return $query->where(['type' =>$document_for,'country_code' => $country_code])->select('id','document_name','expire_on_date','country_code','required');
    }
  }
