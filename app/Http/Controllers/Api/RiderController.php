...
//            ->where('driver_location.car_id','==', 0)
	...
	public function delete_account(Request $request){
        ...
        if($request->user_type=='driver'){
            $result= $this->canDestroy($user_details->id);

            if($result['status'] == 0) {
                return response()->json($result);
            }
            $driver_owe_amount = DriverOweAmount::where('user_id',$user_details->id)->first();

            if(isset($driver_owe_amount) && $driver_owe_amount->amount == 0) {
                $driver_owe_amount->delete();
            }
            try {
                User::find($user_details->id)->delete();
                return response()->json([
                    'status_code' => '1',
                ...
                return response()->json([
                    'status_code' => '0',
                    'status_message' => "This driver has trips, cannot delete",
                ]);
            }
        }else{

            $result= $this->checkPendingTrips($user_details);
            if($result == 0){
                PaymentMethod::where('user_id',$user_details->id)->delete();
                DriverOweAmount::where('user_id',$user_details->id)->delete();
                UsersPromoCode::where('user_id',$user_details->id)->delete();
                Wallet::where('user_id',$user_details->id)->delete();
                ReferralUser::where('user_id',$user_details->id)->orWhere('referral_id',$user_details->id)->delete();
                User::find($user_details->id)->delete();
                return response()->json([
                    'status_code' => '1',
                    'status_message' => __('messages.api.rider_deleted'),
                ]);

            }else{
                return response()->json([
                    'status_code' => '0',
                    'status_message' => __('messages.api.pending_trips'),
                ]);
            }
    ...
    public function canDestroy($user_id)
    {
        ...
        if(LOGIN_USER_TYPE=='company') {
            ...
            if ($user->company_id != Auth::guard('company')->user()->id) {
                $return = ['status' => 0, 'status_message' => 'Invalid ID'];
                return $return;
            }
        }
        ...
        if($driver_trips) {
            $return = ['status' => 0, 'status_message' => 'Driver have some trips, So can\'t delete this driver'];
        }
        else if($user_referral) {
            $return = ['status' => 0, 'status_message' => 'Rider have referrals, So can\'t delete this driver'];
        }
        return $return;
    }
