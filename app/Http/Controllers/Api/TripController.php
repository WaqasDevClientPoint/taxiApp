		...
		if(count($trips)) {
			foreach($trips as $tkey=>$trip) {
		       ...
		       // $data[$tkey]['is_current_trip']= $trip->id==$trip_id;
		        ...
		        $data[$tkey]['invoice'] = $invoice;
		        if($trip->isInstatnt==1){
                    $data[$tkey]['otp_enabled'] = false;
		        }else{
...
\Log::error('trip responce'. json_encode($return_data));
	...
	function instantRide(Request $request){
        ...
        $country_code = !isset($country->phone_code) && empty($country->phone_code)?'251':$country->phone_code;
        $country_id = !isset($country->id) && empty($country->id)?'68':$country->id;
        ...
        $fare_estimation =$this->getFareEstimation($para);

        $data=array(
          'rider_id'=>$user->id,
            'driver_id'=>$user_details->id,
            'fare_estimates'=>$fare_estimation
        );

        if(isset($fare_estimation) && count($fare_estimation)>0){

            $return_data = array_merge([
                'status_code' => '1',
                'status_message' => 'Fare Estimation',
            ],$data);
            ...
            $return_data = array_merge([
                'status_code' => '0',
                'status_message' => "You cannot start Instant ride in this location",
            ]);
            return response()->json($return_data);
    ...
    public function getFareEstimation($para)
    {

        if($para['timezone']) {
            date_default_timezone_set($para['timezone']);
        ...
        $is_schedule = $para['is_schedule'];
        if(isset($para['schedule_date'])!='' && $para['is_schedule']=='1') {
            $day = date('N',strtotime($para['schedule_date']));
            $current_time = $para['schedule_date'].':00';
        }
        ...
        $match_location = Location::select(
            DB::raw("id,status,(ST_WITHIN( ST_GeomFromText('POINT(".$para['pickup_latitude'].' '.$para['pickup_longitude'].")'),ST_GeomFromText(coordinates))) as available ")
        ...
        if($match_location) {
            ...
            $location_cars = ManageFare::where('location_id',$match_location->id)->where('vehicle_id',$para['vehicle_id'])->get()->toArray();

            if(count($location_cars)>0){
                $vehicles 	= array_column($location_cars,'vehicle_id');

                $location_id= $match_location->id;

                $lat_lng_array = [
                    "pickup_latitude" 	=> $para['pickup_latitude'],
                    "pickup_longitude" 	=> $para['pickup_longitude'],
                    "drop_latitude" 	=> $para['drop_latitude'],
                    "drop_longitude" 	=> $para['drop_longitude'],
                ];


                $get_fare_estimation = $this->request_helper->GetDrivingDistance($para['pickup_latitude'], $para['drop_latitude'], $para['pickup_longitude'], $para['drop_longitude']);
                if(isset($get_fare_estimation) && count($get_fare_estimation)>0 ){
                    ...
                    $value = CarType::with(['manage_fare' =>function($q) use ($location_id) {
                    ...
                    $currency = Currency::where('code',$value->manage_fare->currency_code)->first();
    ...
    function startTrip(Request $request){
        $user_details = JWTAuth::parseToken()->authenticate();
        ...
        $req->user_id = $request->rider_id;
        $req->group_id = null;
        $req->seats = $request->seats;
        $req->pickup_latitude = $request->pickup_latitude;
        $req->pickup_longitude = $request->pickup_longitude;
        $req->drop_latitude = $request->drop_latitude;
        $req->drop_longitude = $request->drop_longitude;
        $req->driver_id = $user_details->id;
        $req->car_id = $request->car_id;
        $req->pickup_location = $request->pickup_location;
        $req->drop_location = $request->drop_location;
        $req->payment_mode = $payment_mode;
        $req->status = 'Accepted';
        $req->timezone = $request->timezone;
        $req->schedule_id = 0;
        $req->location_id = $request->location_id;
        $req->additional_fare = 0.00;
        $req->peak_fare = $request->peak_price;
        $req->additional_rider = $request->additional_rider_percentage ?? fees('additional_rider_fare');
        $req->trip_path = $request->trip_path ?? '';
        $req->created_at = date('Y-m-d H:i:s');
        $req->save();

        RideRequest::where('id',$req->id)->update(['group_id'=>$req->id]);


        $fare_estimation_distance = $this->request_helper->GetDrivingDistance($request->pickup_latitude, $request->drop_latitude, $request->pickup_longitude, $request->drop_longitude);
        $peak_price = 0;
        ...
        $request_data = @RideRequest::select('id')->orderBy('id', 'DESC')->first();
        // Create Trip
        $trip = new Trips;
        $trip->user_id 			= $request->rider_id;
        $trip->pool_id 			= 0;
        $trip->pickup_latitude 	= $request->pickup_latitude;
        $trip->pickup_longitude = $request->pickup_longitude;
        $trip->drop_latitude 	= $request->drop_latitude;
        $trip->drop_longitude 	= $request->drop_longitude;
        $trip->driver_id 		= $user_details->id;
        $trip->car_id 			= $request->car_id;
        $trip->pickup_location 	= $request->pickup_location;
        $trip->drop_location 	= $request->drop_location;
        $trip->request_id 		= $req->id;
        $trip->trip_path 		= $request->trip_path;
        $trip->payment_mode 	= $payment_mode;
        $trip->status 			= 'Scheduled';
        $trip->isInstatnt        =  1;
        $trip->currency_code 	= $request->currency_code;
        $trip->peak_fare 		= $peak_price;
        $trip->fare_estimation 	=  $fare_estimation_distance;
        $trip->additional_rider = $additional_rider ?? 0;
        $trip->otp 				= 0;
    ...
    function addRingtone(Request $request){
        $user_details = JWTAuth::parseToken()->authenticate();
        ...
        $ringtone=DB::table('ringtones')->where('driver_id',$request->driver_id)->first();

        if($ringtone){
            $ringtoneInsert=DB::table('ringtones')->where(['driver_id'=>$request->driver_id])->update(['name'=>$request->name]);
        }else{
            $ringtoneInsert=DB::table('ringtones')->Insert(['driver_id'=>$request->driver_id,'name'=>$request->name]);
        }


        $return_data = [
                'status_code' => '1',
                'status_message' => 'Ringtone selected successfully!',
        ];
