...
use App\DataTables\driverVehiclesDataTable;
use App\Models\User;
...
class VehicleController extends Controller
{
    ...
    public function index(VehicleDataTable $dataTable)
    {
        ...
        return $dataTable->setFilter($driver_id)->render('admin.vehicle.view',$data);
    }
    ...
    public function add(Request $request)
    {
        if($request->isMethod('GET')) {
            ...
            $driver_id = request()->id;
            $data['make'] = MakeVehicle::Active()->pluck('make_vehicle_name','id');
            $data['make']->put(0,'Other');
            $data['model'] = VehicleModel::Active()->pluck('model_name','id');
            $data['model']->put(0,'Other');
            ...
            $data['driver']=User::where('id',$driver_id)->first();
...
//                'status'        => 'required',
            ...
            if($request->vehicle_make_id==0){
                $rules['other_make'] = 'required|unique:vehicle_make,make_vehicle_name';
            }

            if($request->vehicle_model_id==0){
                $rules['other_model'] = 'required|unique:vehicle_model,model_name';
            }
...
//                'status'        => trans('messages.driver_dashboard.status'),
            ...
            if($request->vehicle_model_id!=0 && $request->vehicle_make_id!=0){
            ...
            }elseif ($request->vehicle_model_id==0 && $request->vehicle_make_id!=0){
                $make_name = MakeVehicle::whereId($request->vehicle_make_id)->value('make_vehicle_name');
                $vehicle->vehicle_make_id   = $request->vehicle_make_id;
                $vehicle->vehicle_model_id  = 0;
                $vehicle->vehicle_name      = $make_name.' - '.$request->other_model;
            }else{
                $vehicle->vehicle_make_id   = 0;
                $vehicle->vehicle_model_id  = 0;
                $vehicle->vehicle_name      = $request->other_make.' - '.$request->other_model;
            }
        ...
        return redirect(LOGIN_USER_TYPE.'/vehicle/driver/'.$request->driver_id);
    }
        ...
        if($request->submit) {
            ...
            if($vehicle_doc){
                foreach ($vehicle_doc as $key => $value) {
                        ...
                        $document_status = $value->doc_name."_status";
                        if(isset($request->$document_status) && !empty($request->$document_status)){
...
//                        dd($user_doc);
                ...
                foreach ($vehicle_doc as $key => $value) {
                    ...
                    if(isset($user_doc) && !empty($user_doc)){
                        $user_doc->status = $request->$document_status;
                        $expired_date_key = 'expired_date_'.$value->id;
            ...
            return redirect(LOGIN_USER_TYPE.'/vehicle/driver/'.$vehicle->user_id);
        }
    ...
    public function getDriverVehicle(driverVehiclesDataTable $dataTable){
        return $dataTable->render('admin.manage_vehicle.view');
    }
}
