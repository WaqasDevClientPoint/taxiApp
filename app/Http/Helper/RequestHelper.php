...
class RequestHelper
{
	...
	public function push_notification_ios($push_tittle, $data, $user_type, $device_id,$change_title,$firebase_data,$user) {
		...
		try {
			...
			if($push_tittle==__('messages.api.trip_request')){
                $ringtone=DB::table('ringtones')->where('driver_id',$user->id)->first();
                if(isset($ringtone->name)){
                    $notificationBuilder->setBody($push_tittle)->setSound($ringtone->name);
                }else{
	...
	public function push_notification_android($push_tittle, $data, $user_type, $device_id, $change_title = 0,$firebase_data = array()) {
		try{
            logger(json_encode(array("data before:"=>$data)));
            ...
            logger(json_encode(array("firebase_data:"=>$firebase_data)));
            logger(json_encode(array("data:"=>$data)));
            logger(json_encode(array("device_id:"=>$device_id)));
	...
	public function SendPushNotification($user,$push_data)
	{
        ...
        try {
        	if ($device_type == 1) {
				...
				$this->push_notification_ios($push_title, $data, $user_type, $device_id, $change_title,$firebase_data,$user);
				logger("IOS Push");
            }
            else {
				$res=$this->push_notification_android($push_title, $data, $user_type, $device_id, $change_title,$firebase_data);
				logger("Andriod Push");
            }
