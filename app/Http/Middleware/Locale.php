...
class Locale
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if ($session_language) {
            ...
            if(Auth::check())
            {
                $rider_uri = array("signin_rider","signin_corporate", "signin", "signup", "signup_rider","signin_driver", "signin", "signup_driver", "signup_company", "signin_company");

                if (in_array(Request::segment(1), $rider_uri) && Auth::user()->user_type=='Rider')
                {
                  return redirect('/profile');
                }
                if (in_array(Request::segment(1), $rider_uri) && Auth::user()->user_type=='Corporate')
                {
